...
public class CoreMonitor<T> extends Thread implements AppContextAware, EventListener {
    ...
    public void run() {
        ...
        logger.info("coreMonitor is started  , serviceInfo = " + serviceInfo);
        while (true) {
    ...
    public void refresh() {
        ...
        logger.info("CoreMonitor.refresh , ServiceType = " + serviceType);
        //
        //1.提取差异使用
        ...
        for (String nowName : nowNames) {
            if (this.bindNames.containsKey(nowName) == false) {
                RegisterReference<T> register = rsfBinder.rsfService(serviceType, serviceProvider)//
                        .group(nowName).name(serviceType.getName()).version(version).register();
                this.bindNames.put(nowName, register);
                logger.info("register NewCore[{}].", register);
            }
        }
        //
        //3.删除不存在的Core
        ...
        for (String bindName : delete) {
            ...
            logger.info("remove register oldCore[{}].", bindName);
        }
    }
}